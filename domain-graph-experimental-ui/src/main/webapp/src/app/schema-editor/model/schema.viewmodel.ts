...
export class ItemLinkPerspectiveViewModel {
  ...
  static fromAdmin(p: AdminItemLinkPerspective, propertyTypes: PropertyTypeInfo[]): ItemLinkPerspectiveViewModel {
    return new ItemLinkPerspectiveViewModel({
      itemType: p.itemType,
      description: p.description,
      minCardinality: p.minCardinality,
      maxCardinality: p.maxCardinality,
      properties: (p.properties ?? []).map(prop => PropertyDefinitionViewModel.fromAdmin(prop, propertyTypes)),
    });
  }
}
...
export class ItemDefinitionViewModel {
  ...
  static fromAdmin(item: AdminItemDefinition, propertyTypes: PropertyTypeInfo[]): ItemDefinitionViewModel {
    const links: Record<string, ItemLinkPerspectiveViewModel[]> = {};
    for (const [name, perspectives] of Object.entries(item.links ?? {})) {
      links[name] = perspectives.map(p => ItemLinkPerspectiveViewModel.fromAdmin(p, propertyTypes));
    }
