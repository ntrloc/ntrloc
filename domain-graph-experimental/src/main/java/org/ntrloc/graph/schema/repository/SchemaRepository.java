...
public class SchemaRepository {
    ...
    public record PerspectiveRow(UUID id, UUID itemId, UUID linkId, String name, String description, Integer minCardinality, Integer maxCardinality) {}
    ...
    public Set<ItemRow> getAllItems() {
        return Set.copyOf(jdbcClient.sql("SELECT id, name, description FROM schema_item")
                .query((rs, n) -> new ItemRow(rs.getObject("id", UUID.class), rs.getString("name"), rs.getString("description")))
                .list());
    }

    public UUID createItem(String name, String description) {
        return jdbcClient.sql("INSERT INTO schema_item (name, description) VALUES (:name, :description) RETURNING id")
                .param("name", name)
    ...
    public Map<UUID, List<AdminPropertyDefinitionView>> getPropertiesByItem() {
        return jdbcClient.sql("""
                ...
                """)
                .query((rs, n) -> Map.entry(rs.getObject("item_definition_id", UUID.class), mapProperty(rs, n)))
                .list().stream()
    ...
    public Map<UUID, List<AdminPropertyDefinitionView>> getPropertiesByLink() {
        return jdbcClient.sql("""
                ...
                """)
                .query((rs, n) -> Map.entry(rs.getObject("link_definition_id", UUID.class), mapProperty(rs, n)))
                .list().stream()
                .collect(Collectors.groupingBy(Map.Entry::getKey, Collectors.mapping(Map.Entry::getValue, Collectors.toList())));
    }
    ...
    public PerspectiveRow createPerspective(UUID itemId, UUID linkId, String name, String description, Integer minCardinality, Integer maxCardinality) {
        return jdbcClient.sql("""
                ...
                VALUES (:itemId, :linkId, :name, :description, :minCardinality, :maxCardinality) RETURNING *
                """)
                .param("itemId", itemId).param("linkId", linkId).param("name", name).param("description", description)
    ...
    public PerspectiveRow updatePerspective(UUID id, String name, String description, Integer minCardinality, Integer maxCardinality) {
        return jdbcClient.sql("""
                UPDATE schema_item_link_perspective SET name = :name, description = :description, minimum_cardinality = :minCardinality, maximum_cardinality = :maxCardinality
    ...
    public Map<UUID, List<PerspectiveRow>> getPerspectivesByItem() {
        return jdbcClient.sql("SELECT * FROM schema_item_link_perspective ORDER BY item_definition_id, name")
                .query((rs, n) -> Map.entry(rs.getObject("item_definition_id", UUID.class), mapPerspective(rs, n)))
                .list().stream()
                .collect(Collectors.groupingBy(Map.Entry::getKey, Collectors.mapping(Map.Entry::getValue, Collectors.toList())));
    }

    public PerspectiveRow findInversePerspective(UUID linkId, UUID perspectiveId) {
        return jdbcClient.sql("SELECT * FROM schema_item_link_perspective WHERE link_definition_id = :linkId AND id != :perspectiveId")
