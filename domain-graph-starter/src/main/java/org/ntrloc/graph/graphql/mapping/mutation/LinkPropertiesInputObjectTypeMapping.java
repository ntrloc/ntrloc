...
import org.ntrloc.graph.db.language.DateTimeListProperty;
...
public class LinkPropertiesInputObjectTypeMapping implements InputObjectTypeProducer, PropertyInputValueDefinitionMapper {
    ...
    public LinkPropertiesInputObjectTypeMapping(LinkDefinition linkDefinition) {
        ...
        String typeName = String.format("%s %s %s Properties Input", linkDefinition.getSourceItemType(), linkDefinition.getSourceLabel(), linkDefinition.getTargetItemType());
    ...
    List<Property> mapProperties(Map<String, Object> propertiesValue) {
        return propertiesValue.entrySet().stream().map(entry -> {
            ...
            return (Property) switch (propertyType) {
                ...
                case PropertyType.DATE -> new DateProperty(propertyName, (String)propertyValue);
                case PropertyType.DATE_LIST -> new DateListProperty(propertyName, (List<String>)propertyValue);
                case PropertyType.DATETIME -> new DateTimeProperty(propertyName, (Date)propertyValue);
                case PropertyType.DATETIME_LIST -> new DateTimeListProperty(propertyName, (List<Date>)propertyValue);
